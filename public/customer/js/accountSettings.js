...
async function loadAccountSettings() {// [cite: 4]
    try {// [cite: 4]
        ...
        const res = await fetch(`/api/customer/profile?customer_id=${localUser.customer_id}`);// [cite: 4]
        ...
        if (json.status !== 'success' || !json.data) {// [cite: 4]
            ...
            SettingsSwal.fire({// [cite: 4]
                ...
                text: 'Customer account not found in database. Please log in again.'
            }).then(() => {
                window.location.href = 'login.html';
            });
            return;// [cite: 4]
            ...
            try {// [cite: 4]
                ...
                if (data.status === 'success') {// [cite: 4]
                    ...
                    SettingsSwal.fire({// [cite: 4]
                        ...
                        window.location.href = data.redirect || 'login.html';// [cite: 4]
                    });// [cite: 4]
                }
            } catch {// [cite: 4]
                SettingsSwal.fire({// [cite: 4]
